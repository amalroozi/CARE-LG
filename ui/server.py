"""
Minimal local FastAPI backend wrapping the current, corrected CARE-LG
pipeline (src/). Serves the static frontend in this directory and a small
JSON API the frontend calls. Functional, not polished. See
README.md / docs/ for how to start this.
...
# Every import above comes from src/, the single current implementation
# (see archive/investigation_v2_to_v6/experiments_v6_audit/DEEP_AUDIT_RESOLUTION.md
# for the history of why -- the UI used to import from a separate,
# now-archived experiments_v6/lib/ tree).

RESULTS_TABLES_DIR = REPO_ROOT / "results" / "tables"  # where blindspot CSVs live
REPORT_OUTPUT_DIR = REPO_ROOT / "results"               # where generated single-patient reports are written
METRIC = "riemannian"
MU = 0.5
...
def get_run(dataset: str, seed: int):
    ...
    if key not in _cache:
        ...
        blindspot_path = RESULTS_TABLES_DIR / f"{dataset}_blindspot.csv"
...
def recourse(req: RecourseRequest):
    if req.generate_report:
        ...
        report_path = write_report(report_result, REPORT_OUTPUT_DIR)
