"""
Phase 8 of experiments_v6_audit/DEEP_AUDIT.md's resolution (issue #10): the
regression test that would have caught B1 (the entry-gate/interior-gate
asymmetry) had it existed during experiments_v3.
...
deliberately reconstructed COPY of the pre-fix v2/v3 relaxed entry gate
(hard-only, `check_step_horizon=False` -- the exact bug described in
experiments_v6_audit/RECOMMENDATION.md and experiments_v6/CHANGES.md) is
run against the same sampled edges and shown to DISAGREE with the interior
...
Run: .venv/bin/python -m pytest experiments_v6/tests/test_gate_symmetry.py -v
"""
import sys
