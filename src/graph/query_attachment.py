...
from src.graph.constraints_ext import violates as violates_fn, count_violations, _DIRECTIONAL_EPS, _AGE_DECREASE_TOL, _AGE_HORIZON_CAP, _IMMUTABLE_TOL
...
def build_query_edges(
    ...
    Gating convention -- CORRECTED in experiments_v6 (see CHANGES.md, Root
    Cause B1). experiments_v2/v3's original entry gate used
    `gate_violates = hard_count > 0` with `check_step_horizon=False`: hard
    constraints only (immutability + age-non-decrease), no directional
    constraints (BP/cholesterol/glucose direction), no per-step age-horizon
    cap -- while every INTERIOR graph edge was gated by the FULL predicate
    (hard OR directional, check_step_horizon=True,
    `check_clinical_violations` in src/graph/calg.py). This asymmetry let
    entry edges attach through real, measured directional violations (e.g.
    a +35 mg/dL cholesterol increase) that would have pruned an identical
    interior edge. experiments_v6_audit's `verify_entry_gate_fix.py`
    confirmed this by direct measurement (68.8% of entry hops on a UCI
    pilot carried an unpruned directional violation). The entry gate here
    now uses the SAME full predicate as interior edges --
    `gate_violates = (hard_count + dir_count) > 0`, `check_step_horizon=True`
    -- with no separate weaker convention. `v_count` is therefore identical
    to the gate's own severity count (kept as a separate field only for
    interface parity with the soft-penalty sweep).
    """
    N = len(Z_train)
    ...
    # B1 fix: full predicate, matching interior edges exactly (was hard_count>0,
    # check_step_horizon=False -- see corrected docstring above).
    hard_count, dir_count = vectorized_violation_components(unscaled, src_idx, tgt_idx, feature_metadata, check_step_horizon=True)
    gate_violates = (hard_count + dir_count) > 0
