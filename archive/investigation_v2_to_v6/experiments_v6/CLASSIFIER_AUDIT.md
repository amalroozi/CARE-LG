...
Phase 2 of `experiments_v6_audit/DEEP_AUDIT.md`'s resolution (issue #4): the first
independent audit, across all six project sessions, of the `RiskClassifier` MLP's
own predictive validity. Every prior claim in this project (0% CVR, certified
infeasibility, "this patient is high-risk") assumed this classifier's output was
trustworthy without ever checking it directly. All numbers below come from
`experiments_v6/run_classifier_audit.py`, run this session, 5 seeds, both cohorts,
held-out test set. Raw data: `experiments_v6/results/classifier_audit.json`.
...
≥7.5% (2013 ACC/AHA Pooled Cohort Equations — see `experiments_v3/DATA_PROVENANCE.md`
