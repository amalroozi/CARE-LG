# METHOD.md — experiments_v6

CARE-LG v6: the corrected pipeline (B1/B2 fix as the sole foundation), a
neurosymbolic clinical-guideline layer, a three-layer explanation system,
per-patient preference elicitation, and a comparison against FACE and a
PACE-style baseline. All numbers in this document and in `FINDINGS.md` come
from code executed this session (git SHA
`539d87b795b78bb8f9446f99b0fabeded2c84841`); see `CHANGES.md` for exact
reproduction commands.

## Core Assumption (added per `experiments_v6_audit/DEEP_AUDIT.md` issue #6)
...
## 0. Where this code actually lives (Phase 1 of the DEEP_AUDIT resolution)

As of this session, the corrected pipeline, neurosymbolic layer, explanation
system, and preference elicitation are canonically implemented in `src/`
...
`src/data/dataset_registration.py`, `src/pipeline.py`) — not only in
`experiments_v6/lib/`, which is now a set of thin re-export shims pointing
at these `src/` modules (unmodified code, just relocated, so every existing
`experiments_v6` script's imports keep working unchanged). Before this
session, none of the B1/B2 fix, the query-attachment/entry-gate concept
itself, the neurosymbolic layer, or the explanation/preference code existed
anywhere in `src/` — `src/recourse/search.py::find_recourse_path` has no
query-attachment mechanism at all, and this project's whole six-session
investigation lived entirely in parallel `experiments_*/` directories,
disconnected from the actually-importable library
(`experiments_v6_audit/DEEP_AUDIT.md` issue #1). The UI
(`experiments_v6/ui/server.py`) now imports directly from `src/`, so the
demoed code path and the deployable code path are the same code. Full
verification: after the move, the complete 5-seed/both-cohort grid was
rerun end-to-end and produced numbers bit-identical to the pre-move,
already-published `FINDINGS.md` table (38.6%±13.1% UCI / 37.7%±3.4% NHANES
success, 0.00%±0.00% real CVR both cohorts) — see
`experiments_v6_audit/DEEP_AUDIT_RESOLUTION.md` for the full verification
log.

## 1. The corrected pipeline (Phase 1)

`experiments_v6/lib/` adapts `experiments_v3/lib/` with two fixes applied
...
`experiments_v6/run_blindspot.py` ports `experiments_v3/run_blindspot.py`'s
...
`experiments_v6/lib/neurosymbolic.py`, built on `clingo` (ASP). Two kinds of
rules:
...
- `G-STATIN`: 10-year ASCVD risk ≥ 7.5% meets the guideline threshold for
  ...
  `experiments_v3/DATA_PROVENANCE.md` already uses to define this project's
...
- `G-BP-TARGET-DM` / `G-BP-TARGET-NODM`: target BP <130/80 mmHg for patients
  ...
  flag is read from `experiments_v3/data/nhanes_real_full.csv` (present in
...
`experiments_v6/lib/explanations.py`:
...
`experiments_v6/lib/preference.py`. **Documented discrepancy with the
...
**Caveat (per `experiments_v6_audit/DEEP_AUDIT.md` issue #9, checked this
...
(`experiments_v6/CLASSIFIER_AUDIT.md`) found real, respectable discrimination
...
recalibrating: `experiments_v6/CLASSIFIER_AUDIT.md`.

## 6. HTML UI (Phase 7)

See `experiments_v6/ui/` and `HOW_TO_RUN.md`. A minimal local FastAPI
