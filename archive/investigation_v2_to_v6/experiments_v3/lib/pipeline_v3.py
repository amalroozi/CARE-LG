"""
Per-(dataset, seed) model training + encoding pipeline for experiments_v3.
Identical in structure to experiments_v2/lib/pipeline.py, but trains the
Phase B type-aware VAE (experiments_v3.lib.vae_v3) instead of the original
undifferentiated-continuous TabularVAE. The classifier is unchanged (same
src.blackbox_model.RiskClassifier / train_blackbox_model, same
hyperparameters, same as CARE-LG uses -- required for REVISE to share the
...
from experiments_v3.lib.vae_v3 import TabularVAETypeAware, train_vae_type_aware
