"""
experiments_v3 main grid runner (v2's Phase 1+2+3, rerun on corrected data/models):
  - real NHANES cohort (dataset key 'nhanes_real', see DATA_PROVENANCE.md)
    instead of the v2 synthetic stand-in
  - Phase B type-aware VAE decoder (experiments_v3.lib.vae_v3) instead of
    the original undifferentiated-continuous decoder

For each dataset in {uci, nhanes_real} x seed in SEEDS x grid cell in GRID_CELLS:
...
Writes:
  experiments_v3/results/run_metadata.json   (git SHA, seeds, cells, versions)
  experiments_v3/results/{dataset}_per_patient.csv  (long format, all seeds/cells)

Usage:
  .venv/bin/python -m experiments_v3.run_grid --datasets uci nhanes --seeds 0 1 2 3 4
  .venv/bin/python -m experiments_v3.run_grid --datasets uci --seeds 0 --smoke   # fast sanity check
"""
import argparse
import json
import subprocess
...
from experiments_v3.lib.constraints_ext import decode_node, check_decoded_violations, count_violations, violates as violates_fn
