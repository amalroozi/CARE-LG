...
experiments_v2/figures/. matplotlib only, no seaborn.
"""
import json
import sys
...
def fig_a_ablation_bars():
    ...
    for col, dataset in enumerate(["uci", "nhanes"]):
        ...
        ax.set_title(f"{dataset.upper()}: KDE log-density (↑ better)", fontweight="bold")
        ...
        ax.set_title(f"{dataset.upper()}: Clinical effort (↓ better)", fontweight="bold")
...
def fig_b_abstention_decomposition():
    ...
    for ax, dataset in zip(axes, ["uci", "nhanes"]):
        path = RESULTS_DIR / f"{dataset}_blindspot_seed_summary.csv"
        df = pd.read_csv(path)
        if df["n_abstain"].sum() == 0:
            ax.text(0.5, 0.5, f"{dataset.upper()}:\nzero R+hard abstentions\nacross all 5 seeds\n(nothing to decompose)",
                    ha="center", va="center", fontsize=11, transform=ax.transAxes)
            ax.set_xticks([]); ax.set_yticks([])
            ax.set_title(f"{dataset.upper()} abstention decomposition", fontweight="bold")
            continue
        ...
        ax.set_title(f"{dataset.upper()} (mean n={df['n_abstain'].mean():.1f} abstentions/seed)", fontweight="bold")
...
def fig_c_path_geometry(dataset="uci", seed=0, n_examples=3):
    ...
    ax.set_title(f"Fig C — R+hard vs E+hard path geometry, {dataset.upper()} seed {seed}\n"
...
def fig_d_soft_violation_curve():
    ...
    for dataset, color in [("uci", "#3498db"), ("nhanes", "#e74c3c")]:
        df = pd.read_csv(RESULTS_DIR / f"seedcell_{dataset}.csv")
        means, stds = [], []
        for lam in lambdas:
        ...
        ax.errorbar(lambdas, means, yerr=stds, marker="o", color=color, label=f"{dataset.upper()} R+soft(λ)", capsize=4)
        ax.axhline(hard_mean, color=color, linestyle=":", alpha=0.7, label=f"{dataset.upper()} R+hard (reference)")
    ax.set_xscale("log")
