...
in `archive/investigation_v2_to_v6/experiments_v3/data_raw/` (not deleted, kept for audit).
...
CSV (`archive/investigation_v2_to_v6/experiments_v3/data/nhanes_real.csv`):
...
logistic function of risk factors (see `archive/investigation_v2_to_v6/experiments_v2/REPO_MAP.md`) — not a validated score.
...
Full table: `archive/investigation_v2_to_v6/experiments_v3/data/missingness_report.csv`.
...
Full model-facing CSV: `archive/investigation_v2_to_v6/experiments_v3/data/nhanes_real.csv` (7 features + target, exactly
matching the existing schema). Full CSV with provenance columns (race group, computed
ASCVD%, smoker/diabetic/treated-BP flags) retained for audit: `archive/investigation_v2_to_v6/experiments_v3/data/nhanes_real_full.csv`.
...
**separate** dataset key, `'nhanes_real'` (see `archive/investigation_v2_to_v6/experiments_v3/lib/dataset_registration.py`),
at runtime, without editing `configs/dataset_config.py`. Every output filename, config
label, and figure/table row in `archive/investigation_v2_to_v6/experiments_v3/` says `nhanes_real` or "real NHANES," never
